<script lang="ts">
	...
	import BrowserLocationDisabled from '$components/browser-location-disabled.svelte';
	...
	// let ready = true;
	$: {
		console.log('else if', !!$currentLocation.error && !$currentLocation.locationServicesEnabled);
	}
</script>

{#if $currentLocation.loading}
	<div>Loading....</div>
{:else if !!$currentLocation.error && !$currentLocation.locationServicesEnabled}
	<BrowserLocationDisabled />
{:else}
	<div class="pt-5 pb-16">
		<div class="font-semibold font-anybody py-3 text-4xl text-center title-blue">
			Request a ride
		</div>
		<div class="p-2">
			<StartStopForm
